...
import kr.or.hsnarae.transporthelp.impl.net.json.rep.REP_JSONCallHistory;
import kr.or.hsnarae.transporthelp.impl.net.json.rep.REP_JSONReqCallCancel;
import kr.or.hsnarae.transporthelp.impl.net.json.req.JSON_REQCallHistory;
...
import kr.or.hsnarae.transporthelp.impl.rowadaptors.ListHistoryRowAdaptor;
...
//        doHistoryListView ();
    ...
    private void doHistoryListView() {
        Log.d("테스트", "현재 리스트 확인 : " + mCallHistory.size());
    ...
    public void jsonMessageEvent(JSONMessageEvent event) {
        ...
        Log.d("테스트", "jsonMessageEvent 수신 상태 : " + event.getMessageStatus());
        Log.d("테스트", "jsonMessageEvent 수신 상태 : " + event.getPageName());
    ...
    private void receiveCallHistory(JSONMessageEvent event) {
        ...
        if (json.getBody().getResult().equalsIgnoreCase(getResources().getString(R.string.success))) {
            ...
            Log.d("테스트", "receiveCallHistory success");
        } else {
            // 실패 사유
            Logview.Logwrite(THIS_TAG, "receiveCallHistory false : " + json.getBody().getCause());
            Log.d("테스트", "receiveCallHistory false" + json.getBody().getCause());
            handler.sendEmptyMessage(MSG_HTTP_FAILED);
        }
        ...
        if (list != null && list.size() > 0) {
            ...
            for (REP_JSONCallHistory.Body.CallInfoItem call : list) {
                ...
                //180410 송명진 - 나의 이용 내역에서 서버 요청시에 출발지점이 널값인 경우가 있는데 (어떠한 오류로 인하여)
                //그랬을 경우 return시켜서 안보여주는 것이 아니라 그 항목은 넘기고 다른 항목들은 보여줄 수 있도록 수정
