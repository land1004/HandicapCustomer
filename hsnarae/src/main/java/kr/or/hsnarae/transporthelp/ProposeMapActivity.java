...
import kr.or.hsnarae.transporthelp.common.util.TimePickerDailog;
import kr.or.hsnarae.transporthelp.common.widget.SegmentedButton;
...
import kr.or.hsnarae.transporthelp.impl.net.json.rep.REP_JSONCallAvailableTime;
import kr.or.hsnarae.transporthelp.impl.net.json.rep.REP_JSONNoticeList;
import kr.or.hsnarae.transporthelp.impl.net.json.rep.REP_JSONReqCall;
import kr.or.hsnarae.transporthelp.impl.net.json.rep.REP_JSONReservReqCall;
import kr.or.hsnarae.transporthelp.impl.net.json.rep.REP_JSONReverseGeocoding;
import kr.or.hsnarae.transporthelp.impl.net.json.rep.REP_JSONUseTypeList;
import kr.or.hsnarae.transporthelp.impl.net.json.req.JSON_REQCallAvailableTime;
import kr.or.hsnarae.transporthelp.impl.net.json.req.JSON_REQNoticeList;
import kr.or.hsnarae.transporthelp.impl.net.json.req.JSON_REQReqCall;
import kr.or.hsnarae.transporthelp.impl.net.json.req.JSON_REQReservReqCall;
import kr.or.hsnarae.transporthelp.impl.net.json.req.JSON_REQReverseGeocoding;
import kr.or.hsnarae.transporthelp.impl.net.json.req.JSON_REQUseTypeList;
...
import kr.or.hsnarae.transporthelp.impl.popup.DialogBookmark;
import kr.or.hsnarae.transporthelp.impl.popup.DialogReceiveHandler;
import kr.or.hsnarae.transporthelp.impl.preference.ConfigPreference;
import kr.or.hsnarae.transporthelp.impl.preference.OrderAvailableTimePreference;
        ...
        implements View.OnClickListener, DatePickerDialog.OnDateSetListener, TimePickerDialog.OnTimeSetListener
{
    ...
    private ArrayList<ITEM> mUseType;
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        // 공지사항 180207 송명진 공지사항 개편으로 인하여 새로운 공지가 있다면 다른 아이콘을 보여준다.
        if (checkNewNotice() != 0) {
            textView.setBackground(ContextCompat.getDrawable(this, R.drawable.noticealram48));
            textView.setText("");
        }
    ...
    protected void onResume()
    {
        ...
        TextView textView = (TextView)findViewById(R.id.btnNotice);
        // 공지사항 180207 송명진 공지사항 개편으로 인하여 새로운 공지가 있다면 다른 아이콘을 보여준다.
        if (checkNewNotice() != 0) {
            textView.setBackground(ContextCompat.getDrawable(this, R.drawable.noticealram48));
            textView.setText("");
        }else {
            textView.setBackground(ContextCompat.getDrawable(this, R.drawable.button_background));
            textView.setText(R.string.fa_bell_o);
        }
...
//        tv.setOnClickListener(this);
    ...
    private void initOrder ()
    {
        ...
        if (select != null && select.getCount() > 0)
        {
            if (mUseType != null)
                mUseType.clear();
            mUseType = null;
            mUseType = new ArrayList<ITEM>();
            ITEM data;
            select.moveFirst();
            do {
                data = new ITEM(select.getValue(DBSchema.COL_NAME), select.getValue(DBSchema.COL_CODE));
                mUseType.add(data);
    ...
    /**
     * 출/도착지 설정값을 지운다.
     * 즉시, 예약 배차 변경으로 배차 불가지역 설정이 가능하기 때문에
     */
    ...
    public void onClick(View view)
    {
        switch (view.getId())
        {
            ...
            case R.id.btnCenter:        // 접수하기
                if (((HSNaraeApplication)getApplication()).checkReorder()) {
                    doAlertDialogView("알림", getResources().getString(R.string.reorder));
                } else {
                    doOrder();
                }
                break;
        ...
        public void handleMessage(Message msg)
        {
            switch (msg.what)
            {
                ...
                case MSG_HTTP_FAILED:
                    Toast.makeText(getApplicationContext(), getString(R.string.http_failed),Toast.LENGTH_LONG).show();
                    break;
            }
        }
    };
...
//                    Toast.On("Please enable a My Location source in system settings");
    ...
    public void jsonMessageEvent(JSONMessageEvent event)
    {
        ...
        Log.d("테스트","jsonMessageEvent 수신 상태 : " + event.getMessageBody());
        if (event.getMessageStatus()) {
            ...
            if (pageName != null) {
                ...
                else if (pageName.trim().toLowerCase().equalsIgnoreCase(IJSONPageMethod.JSON_PAGE_METHOD.UseTypeList.name().toLowerCase())) {
                ...
                else if (pageName.trim().toLowerCase().equalsIgnoreCase(IJSONPageMethod.JSON_PAGE_METHOD.NoticeList.name().toLowerCase())){
                    stopProgress();

                    checkNewNotice(event);
                }
    ...
    private void doUseType ()
    {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                JSON_REQUseTypeList reqvalue = new JSON_REQUseTypeList();
                RetrofitProcessManager.doRetrofitRequest(JSON_REQUseTypeList.PAGE_NAME, reqvalue.getParams());
            }
        }).start();
    }
    ...
    private void receiveUseTypeList (JSONMessageEvent event)
    {
        ...
        final REP_JSONUseTypeList json = gson.fromJson(event.getMessageBody(), REP_JSONUseTypeList.class);
        ...
        if (json.getBody().getResult().equalsIgnoreCase(getResources().getString(R.string.success)))
        {
            ...
            new Thread(new Runnable() {
                @Override
                public void run() {
                    List<REP_JSONUseTypeList.Body.UseType> list = json.getBody().getUseTypes();
                    if (list != null && list.size() > 0)
                    {
                        // 기존에 있는 모든 자료 삭제
                        ...
                        // 새포운 자료 입력
                        for (REP_JSONUseTypeList.Body.UseType item : list)
                        {
                            String sql = String.format("INSERT INTO %s (name, code) VALUES ('%s','%s');", DBSchema.TABLE_NAME_USETYPE, item.getUsetype(), item.getUsetypecode());
                            DBControlManager.dbTransaction(sql);
                        }
    ...
    /**
     * DB에 데이터가 없다면 저장 있다면 최근 공지사항 번호랑 비교후 새로운 공지사항이 있다면 알림을 띄워준다.
     */
    private ArrayList<NoticeItem> mNoticeList; //공지사항 리스트 저장할 변수
    private void checkNewNotice(JSONMessageEvent event) {
        ...
        if (DBControlManager.dbTransaction(DBSchema.TABLE_CREATE_NOTICELIST))
            Logview.Logwrite(THIS_TAG, "테이블 생성 되었습니다.");
        else
            Logview.Logwrite(THIS_TAG, "테이블 생성 실패 되었습니다.");
...
//        sql = "delete from "+DBSchema.TABLE_NOTICE_LIST+";";
//        if (DBControlManager.dbTransaction(sql))
//            Logview.Logwrite(THIS_TAG, "저장 되었습니다.");
//        else
//            Logview.Logwrite(THIS_TAG, "저장이 실패 되었습니다.");
        ...
        Collections.sort(list, new CompareSeqDsc());

        int lastSeq; // 마지막으로 올라온 공지사항의 고유번호 변수
        try{
            select.moveLast();
            lastSeq = Integer.parseInt(select.getValue(1));
        ...
        if (list != null && list.size() > 0) {
            ...
            NoticeItem notice;
            select.moveFirst(); //마지막 공지사항에 갔던 커서 다시 처음으로...
...
//                Log.d("테스트","현재의 마지막 seq : " + lastSeq + "현재 노티스 seq : " + notice.title);
//                Log.d("테스트","현재의 마지막 seq : " + lastSeq + "현재 노티스 seq : " + notice.seq);
//                Log.d("테스트","현재의 마지막 seq : " + lastSeq + "현재 노티스 seq : " + notice.date);

                if(lastSeq < Integer.parseInt(notice.seq)) {     //고유번호보다 큰 게시글만 DB에 추가한다.

                    sql = "INSERT INTO " + DBSchema.TABLE_NOTICE_LIST
                            + " ("
                            + DBSchema.COL_NOTICE_SEQ + ", "
                            + DBSchema.COL_NOTICE_TITLE + ", "
                            + DBSchema.COL_NOTICE_DATE + ", "
                            + DBSchema.COL_READYN
                            + ") "
                            + " VALUES ('"
                            + notice.seq + "','"
                    ...
                    if (DBControlManager.dbTransaction(sql))
                        Logview.Logwrite(THIS_TAG, "input DB");
                    else
                        Logview.Logwrite(THIS_TAG, "Failed");

                }

//                Log.d("테스트","셀렉트 값0 확인 : "+ select.getValue(0) );
//                Log.d("테스트","셀렉트 값1 확인 : "+ select.getValue(1) );
//                Log.d("테스트","셀렉트 값2 확인 : "+ select.getValue(2) );
//                Log.d("테스트","셀렉트 값3 확인 : "+ select.getValue(3) );
                select.moveNext();
            }
    ...
    public int checkNewNotice(){
        String sql = "SELECT * FROM " + DBSchema.TABLE_NOTICE_LIST+" WHERE "+ DBSchema.COL_READYN + " = 'N'" + ";";
        SelectHelper select = DBControlManager.dbSelect(sql);
        return select.getCount();
    }

    /**
     * 180220 송명진 - 오름차순 정렬 (DB에 입력할때 오름차순으로 리스트에 넣기 위해서 구현함)
     */
    static class CompareSeqAsc implements Comparator<REP_JSONNoticeList.Body.NOTICE>{

        @Override
        public int compare(REP_JSONNoticeList.Body.NOTICE o1, REP_JSONNoticeList.Body.NOTICE o2) {
            return Integer.parseInt(o1.noticeSeq) > Integer.parseInt(o2.noticeSeq) ? -1 : Integer.parseInt(o1.noticeSeq) < Integer.parseInt(o1.noticeSeq) ? 1:0;
        }
    }

    private class CompareSeqDsc implements Comparator<REP_JSONNoticeList.Body.NOTICE> {
