package kr.or.hsnarae.transporthelp.impl.popup;
...
import kr.or.hsnarae.transporthelp.impl.rowadaptors.ListBookmarkRowAdaptor;
    ...
    private void doSendParent (boolean flag, boolean ordertype)
    {
        ...
        if (flag) {
            ...
            if (!ordertype) {
                // 2018.06.03
                // 즉시 배차는 화성시 관내만 된다.
                if (item.endAddr.contains("화성시") == false) {
                    Toast.makeText(getContext(), getContext().getResources().getString(R.string.smart_alloc_msg), Toast.LENGTH_LONG).show();
                    return;
                }
            }
